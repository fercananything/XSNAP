...
Positional arguments:
    ...
    - ``EVT_FILE`` :  Path to the Chandra ``evt2`` event file.

Options:
    - ``--src-reg SRC_REG`` : Existing source region file.

    - ``--bkg-reg BKG_REG`` : Existing background region file.

    - ``--outdir OUTDIR`` : Output directory. Defaults to the event file directory.

    - ``--ra RA`` : Source RA in degrees. Required only when auto-generating regions.

    - ``--dec DEC`` : Source Dec in degrees. Required only when auto-generating regions.

    - ``--src-radius SRC_RADIUS`` : Auto source radius in arcsec. Default: ``2``.

    - ``--bkg-inner BKG_INNER`` : Auto background annulus inner radius in arcsec. Defaults to ``--src-radius``.

    - ``--bkg-outer BKG_OUTER`` : Auto background annulus outer radius in arcsec. Default: ``45``.

    - ``--bkg-circle-radius BKG_CIRCLE_RADIUS`` : Circular background radius in arcsec, used when a detection falls inside the proposed background annulus. Defaults to    - ``--bkg-outer``.
